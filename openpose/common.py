...
def cal_distance(cts1, cts2):
    ...
    for i in range(CocoPart.Background.value):
        if i in key1 and i in key2:
            count += 1
            dis += ((cts1[i][0] - cts2[i][0])**2 + (cts1[i][1] - cts2[i][1])**2)**0.5
    if count == 0:
        dis = 1e7
    else:
        dis /= count
...
def get_best_joints(img, human_list, pre_centers):
    image_h, image_w = img.shape[:2]
    cover = False
    all_centers = []
    conf = []
    ...
    for human in human_list:
        part_idxs = human.keys()
        centers = {}
        conf.append(cal_confidence(human))
        for i in range(CocoPart.Background.value):
            if i not in part_idxs:
                continue
            part_coord = human[i][1]
    ...
    if len(pre_centers) == 0:
        cover = True
        index = -1 if len(conf) == 0 else np.argmax(conf)
        best_joints = all_centers[index] if index >= 0 else {}
    else:
