...
trg_train_fmap = root + train + 'JTM_InceptionV3_shared/' + str(args.frame) + '/' + args.layer + '/'
trg_test_fmap = root + test + 'JTM_InceptionV3_shared/' + str(args.frame) + '/' + args.layer + '/'
...
def run(scope, model):
    ...
    for file in file_list:
        x_ori = (cv2.resize(cv2.imread(path_ori + file), (224, 224)))
        x_mc = (cv2.resize(cv2.imread(path_mc + file), (224, 224)))
