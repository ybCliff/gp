...
if K.image_data_format() == 'channels_first':
    input_shape = (1, 16, 32)  # image shape
else:
    input_shape = (16, 32, 1)  # image shape
num_classes = 51  # number of classes
epochs = 3

def preprocess_input(x):
    return x.astype('float32').reshape((-1,) + input_shape)
...
train_spatial_root = root + args.dataset + '_train_test_splits/' + train + '_spatial/block5_pool/'
test_spatial_root = root + args.dataset + '_train_test_splits/' + test + '_spatial/block5_pool/'
...
def load_data(path):
    ...
    for file in file_list:
        ...
        tmpx = np.array([float(i) for i in tmp.split(',')])
        file_to_read.close()

        x.append(tmpx.reshape((16, 32)))
...
print('Loading',args.dataset, args.split, 'spatial data...')
